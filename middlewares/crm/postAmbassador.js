...
const { newAmbassador } = require("../../services/universityLeadsServices");

axios.defaults.baseURL = process.env.BASE_URL;

const postAmbassador = async (req, res, _) => {
  const postRequest = [
    {
  ...
  try {
    ...
    return res
      .status(201)
      .json(await newAmbassador({ ...req.body, crmId: crmLeadId }));
  } catch (error) {
    return res.status(400).json(error);
  }
};

module.exports = postAmbassador;
