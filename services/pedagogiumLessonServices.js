const PedagogiumLessons = require("../db/models/pedagogiumLessonModel");

const newLesson = async (body) => {
  const lesson = new PedagogiumLessons(body);
...
const newQuestion = async (lessonId, body) => {
  const lesson = await PedagogiumLessons.findById(lessonId);
...
const findLessonsByGroup = async (group) =>
  await PedagogiumLessons.find({ page: group });
...
const findLessonsByGroupAndDate = async (group, date) => {
  ...
  const lessons = await PedagogiumLessons.find({
    page: group,
