const UniLessonResults = require("../db/models/uniLessonResults");

const newLesson = async (body) => {
  const lesson = new UniLessonResults(body);
...
const newQuestion = async (lessonId, body) => {
  const lesson = await UniLessonResults.findById(lessonId);
...
const findLessonsByGroup = async (group) =>
  await UniLessonResults.find({ page: group });
...
const findLessonsByGroupAndDate = async (group, date) => {
  ...
  const lessons = await UniLessonResults.find({
    page: group,
